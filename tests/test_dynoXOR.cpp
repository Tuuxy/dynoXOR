...
TEST_CASE("verifyFile() validates file existence and content","[file],[verify]")
{
...
TEST_CASE("processFileInChunks performs XOR encryption", "[xor][processing]")
{
    ...
    {
        REQUIRE_THROWS_AS(processFileInChunks("non_existent.txt", outputFile, key, 1024), std::runtime_error);
    }
...
TEST_CASE("logKey writes keys to log file","[logging]")
{
    ...
    SECTION("Creates log file and logs key")
    {
        ...
        std::string configDir { getConfigDir() };
        std::filesystem::path logPath { std::filesystem::path(configDir) / Constants::logFileName };
